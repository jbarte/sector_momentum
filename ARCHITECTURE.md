...
`data_export.py`, `band_fixture.py`.
...
- **`docs/data.json`** — machine-readable export (`schema_version` 2 adds a
  config-only `config` block); gated like the board.
- **`docs/band-fixture.json`** — golden parity vectors for the iOS app,
  generated by `dashboard/band_fixture.py` from the real Python rules.

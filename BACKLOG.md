...
times, unescaped `innerHTML` interpolation in `auth.js`/`scan-digest.js`, each
price parquet read twice per scan) shipped 2026-08-23 — see Done. What
remains here is the audit record itself: negative findings with no other item
to attach to, kept so the same sweep is not repeated.
...
- **Trailing stop-loss on held positions** — a theme you hold that closes 12%
  ...
  the leaderboard. Threshold picked by a 9x2x2x2 sweep (nine thresholds, both
  presets, 70 and 100 bps, two disjoint windows): cost level reorders nothing,
  the effect decays smoothly to baseline, and 8% is where it turns negative
  (`long` on 2022- earned 19.6% against a 20.6% no-stop baseline). 12% over
  the marginally better-scoring 10% for margin from that cliff, since the live
  rule runs on star dates rather than the model entry dates the sweep
  measured. Latching (fires once; unstarring clears it via the
  `position_stops` FK cascade), opt-in per user via `alert_prefs
  .stop_loss_since`, and never auto-unstars. Spec:
  `sector_momentum-notes/specs/2026-09-07-live-trailing-stop-loss-design.md`.
  **Post-merge manual step: run `scripts/position_stops_migration.sql` and
  `scripts/stop_loss_pref_migration.sql` in the Supabase SQL editor.**

## Market Context chips replaced with 1M/12M performance vs ACWI (2026-09-05)

The SPY/VIX chips described a market the reader doesn't trade. Cell C now
shows the selected horizon's excess return over ACWI across two windows,
read from the backtest equity curve that already existed — no new
computation, no new fetch.

**Two windows, not one, deliberately.** The window picks the story: measured
2026-09-05, `medium` read +1.2pp over 1M, −11.2pp over 3M, and +32.2pp over
12M — a 43pp swing in narrative from window choice alone. Showing both 1M
and 12M costs no more than one and makes their disagreement visible, rather
than inviting the reader to over-update on an arbitrary framing.

**An earlier draft proposed the full-history CAGR here and was rejected:** a
figure that never changes between builds is wallpaper in a daily-read strip,
and it answers a trust question that already has a home in the Backtest tab.

**The trap this could have shipped with:** `engine.py` appends one calendar
date past the final rebalance, so the curve's last point is a stub. Index
arithmetic gives a one-day return rendered labelled "1M" (−2.0% instead of
+1.2pp). Windows are anchored by date, and
`test_window_excess_anchors_by_date_not_by_index` pins it.

Staleness is surfaced rather than hidden — `summary.json` refreshes only on
a manual `backtest.py` run, so past 35 days the chips show the as-of date
inline instead of silently disappearing.

Also removed: `dashboard/macro.py` and its fetch, the header's duplicate
chip block (the Sentiment page needed no replacement — `markLive()` never
fired there), the VIX-band CSS, and the dead `macro_*` /
`market_context_*` i18n keys.

Spec: `sector_momentum-notes/specs/2026-09-05-strategy-track-record-chip-design.md`
...
panel's *intent* (guest sees a bare date, no book-specific controls) was
already correct; these were two independent implementation slips leaking
signed-in-only controls anyway.

1. **CSS specificity.** `.rp-lock { display: inline-flex; ... }` in
   `_tables.css.j2` beat the UA `[hidden] { display: none }` rule —
   `renderReviewPanel()` correctly set `lockWrap.hidden`, but the "Lock book
   until the review" checkbox rendered anyway. Fixed with an explicit
   `.rp-lock[hidden] { display: none; }` override, following the same
   pattern already used for `.filter-group[hidden]` and four other
   `display`-bearing classes in this codebase.
2. **JS ordering.** `renderReviewPanel()` set `doneBtn.hidden = !status.due`
   before the guest early-return, so the `Done ✓` button unhid for guests
   too whenever a review was due. Fixed by defaulting `doneBtn.hidden = true`
   up front and only setting it from `status.due` after both early returns
   (stale calendar, guest) — so it unhides only once a real book exists.

Verified live in the built dashboard as a signed-out guest on a day with
`status.due === true` ("Review due · Aug 31"): both `lockWrap` and `doneBtn`
now report `hidden === true` and `getComputedStyle(...).display === "none"`,
and the panel visually shows only the bare due-date line.
...
(1 wasted pass). Rather than patching each pairwise call site — the same
scaling problem as the guard pattern the star-tap fix used — coalesced at
the actual point of redundant cost: `renderMobileCards()` itself now wraps
its rebuild in a dirty-flag + microtask-flush coalescer, so any number of
calls within one synchronous tick collapse into a single real rebuild. Every
one of the four call sites converges on this same function (traced and
tabled in the design spec), so the fix needed zero changes to auth.js,
scan-history.js, or applyRanking() — only `renderMobileCards()` in
index.html.j2 changed.

Also added this bug class's first real automated regression test
(`tests/test_dashboard_render_coalescing.py`, Playwright/Python): previously
this was only ever caught by live, uninstrumented browser counting. The test
counts actual writes to `#leaderboard-cards`'s `innerHTML` rather than
naming an internal function, so it works unchanged whether the fix is
present or not.

See `sector_momentum-notes/specs/2026-08-25-mobile-render-coalescing-design.md`
and `sector_momentum-notes/plans/2026-08-25-mobile-render-coalescing.md`.
...
Grouped, from the 2026-08-23 project sweep — none individually justified its
own PR.

**`_modal.js.j2` was inlined three times per page** (~12 KB). The
`window.SMModal = window.SMModal || (...)` guard made it idempotent, not a
bug, but three copies of a focus-trap implementation that had to stay
byte-identical. Both `index.html.j2` and `sentiment.html.j2` already included
it once directly, ahead of `_footer.html.j2` and `_methodology.html.j2` in
document order — removed the two redundant includes inside those partials,
which now rely on `window.SMModal` already existing.

**Unescaped `innerHTML` interpolation** in `auth.js`'s `renderLatestRows()`
and `scan-digest.js`'s `fmtChip()` — both interpolate theme/sector names by
string concatenation with no escaping. Not exploitable today (names come from
`config/themes.yaml` via the pipeline, never from a reader), hardening only.
Added a local `escapeHtml()` to each file (no shared always-loaded JS module
exists to put one helper in — matches the codebase's existing per-file IIFE
convention) and wrapped the two interpolation sites.

**Each price parquet was read twice per scan.** `_cache_is_fresh` did a full
`pd.read_parquet` just to inspect the index; `fetch_prices` then immediately
read the identical file again on every cache hit — 20 tickers × 2 reads.
Renamed to `_load_fresh_cache`, returning the DataFrame it already loaded
(`None` when stale) instead of a bare bool. `_cache_is_fresh` stays as a thin
bool wrapper (`_load_fresh_cache(...) is not None`) — kept specifically
because it already had ~20 pinned tests exercising every freshness-boundary
edge case (weekend bridges, holiday gaps, the 2026-08-09 Thursday-twice bug),
and rewriting all of them for a return-type change would have been a much
...
Verified live in a browser, both pages: `window.SMModal` exists globally,
opening the methodology modal moves focus inside it (focus trap engaged), and
a dispatched `Escape` keydown closes it — on `sentiment.html.j2` too, proving

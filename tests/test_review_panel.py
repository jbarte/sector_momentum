...
def test_panel_states_the_consequence_not_only_the_date():
    ...
    """
    js = _TPL.read_text()
    assert "No action until then." in js, (
...
def test_panel_explains_an_unbuyable_blocked_slot():
    ...
    js = _TPL.read_text()
    assert "blocked" in js and "Slot stays empty" in js, (
...
def test_panel_names_the_no_changes_state_explicitly():
    ...
    assert "No changes — your book already matches." in _TPL.read_text() + _PANEL.read_text()
...
def test_panel_handles_an_exhausted_review_calendar():
    ...
    assert "Review calendar is out of date" in _TPL.read_text(), (
